		...
		parse : function( bbcode )
		{
			...
			while ( ( parts = this._.bbcPartsRegex.exec( bbcode ) ) )
			{
				...
				// Closing tag
					...
					this.onTagClose( bbcodeToHtml[ part ] );
			}
	...
	CKEDITOR.htmlParser.fragment.fromBBCode = function( source )
	{
		...
		function checkPendingBrs( tagName, closing )
		{
			var len = currentNode.children.length,
				...
				lineBreakParent = !previous && BBCodeWriter.getRule( htmlToBBCode[ currentNode.name ], 'breakAfterOpen' ),
				lineBreakPrevious = previous && previous.type == CKEDITOR.NODE_ELEMENT && BBCodeWriter.getRule( htmlToBBCode[ previous.name ], 'breakAfterClose' ),
				lineBreakCurrent = tagName && BBCodeWriter.getRule( htmlToBBCode[ tagName ], closing ? 'breakBeforeClose' : 'breakBeforeOpen' );
			  ...
			  editor.dataProcessor.htmlFilter.addRules(
			  {
				elements :
				{
					$ : function( element )
					{
						...
						// Direct mapping
						else if ( tagName in htmlToBBCode )
							tagName = htmlToBBCode[ tagName ];
					...
					filters.push( function( element )
						{
							var htmlName = element.getName(),
								name = htmlToBBCode[ htmlName ] || false;
